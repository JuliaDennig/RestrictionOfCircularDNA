...
class BandSizes:
    def __init__(self):
        ...
        self.open_ape_file = self.find_bindings.open_ape_file()
        self.open_enzyme_csv = self.find_bindings.open_enzyme_csv()
        self.generate_alternative_sequence = self.find_bindings.fuse_beginning_end_of_sequence(self.open_ape_file)
        self.find_binding_sites = self.find_bindings.find_binding_sites(self.open_ape_file, self.open_enzyme_csv)
    ...
    def calculate_band_sizes_one_enzyme(self):
        for i in range(len(self.enzyme_list_filtered)):
            ...
            if len(self.enzyme_list_filtered[i].binding_sites) == 1:
                ...
                self.enzyme_list_filtered[i].band_sizes_one_enzyme = band_sizes
                ...
                band_sizes.append(len(self.open_ape_file) - self.enzyme_list_filtered[i].binding_sites[-1] +
        ...
        for i in range(len(combined_enzymes_list)):
            ...
            if len(combined_enzymes_list[i].binding_sites) == 1:
                ...
                for j in range(len(combined_enzymes_list[i].binding_sites) - 1):
                    ...
                    band_sizes.append(band)
                band_sizes.append(len(self.open_ape_file) - combined_enzymes_list[i].binding_sites[-1] +
...
found_bindings = BandSizes()
found_bindings.calculate_band_sizes_one_enzyme()
combined_bindings_list = found_bindings.combine_binding_sites_two_enzymes()
found_bindings.calculate_band_sizes_two_enzymes(combined_bindings_list)
